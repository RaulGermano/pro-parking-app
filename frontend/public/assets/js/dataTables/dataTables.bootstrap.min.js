...
(function(b) {
	...
	b.extend(!0, f.defaults, {
		dom:
			"<'row'<'col-6'l><'col-6 text-right'f>><'row'<'col-sm-12'tr>><'row'<'col-sm-12 col-md-5'i><'col-sm-12 col-md-7 'p>>",
		renderer: 'bootstrap'
	});
	...
	f.ext.renderer.pageButton.bootstrap = function(a, h, r, s, j, n) {
			...
			q = function(d, f) {
				...
				for (h = f.length; l < h; l++)
					if (((c = f[l]), b.isArray(c))) q(d, c);
						...
						$('.page-link').css(
							'box-shadow',
							'0 .2rem 1rem rgba(0,0,0,.1)'
						);
						$('.active a').css('color', 'black');

						$('.active a').css('color', 'white');
						$('.active a').css('background-color', '#dc3545');
						$('.active a').css('border-color', '#dc3545');
						$('.active a').css(
							'box-shadow',
							'0 .2rem 1rem rgba(0,0,0,.1)'
						);
		...
		q(
			b(h)
				.empty()
				.html('<ul class="pagination d-flex justify-content-end"/>')
				.children('ul'),
			s
		);
		i !== m &&
